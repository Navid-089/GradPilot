...
export default function ProfilePage() {
    ...
    const fetchData = async () => {
      try {
        ...
        const [profileResponse, countries, majors, interests] = await Promise.all([
          fetch(`http://localhost:8182/api/v1/auth/profile?email=${currentUser.email}`, { headers }),
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:8182/api/v1/auth/profile?email=${userEmail}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
