...
export default function UniversitiesPage() {
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      try {
        ...
        console.log(
          "Countries not in data:",
          countries.filter((c) => !uniqueCountries.includes(c))
        );
        console.log(
          "Countries in data but not in filter:",
          uniqueCountries.filter((c) => !countries.includes(c))
        );
  ...
  useEffect(() => {
    ...
    if (debouncedSearchQuery && debouncedSearchQuery.trim()) {
      ...
      console.log(
        `Search filter: ${beforeSearch} -> ${result.length} universities`
      );
    }
    ...
    if (
      ...
      filters.countries.length < countries.length
    ) {
      console.log(
        "Applying country filter. Selected countries:",
        filters.countries
      );
      console.log("Available countries in data:");
      const uniqueCountriesInResult = [
        ...new Set(result.map((uni) => uni.country || "Unknown")),
      ].sort();
      console.log(
        "Unique countries in current result:",
        uniqueCountriesInResult
      );
      ...
      result = result.filter((uni) => {
        ...
        if (!matchesFilter) {
          console.log(
            `University "${uni.name}" with country "${uniCountry}" filtered out`
          );
        }
        return matchesFilter;
      });
      console.log(
        `Country filter: ${beforeFilter} -> ${result.length} universities`
      );

      // If no universities remain after filtering, there might be a country name mismatch
      if (result.length === 0 && beforeFilter > 0) {
        console.warn(
          "⚠️ NO UNIVERSITIES AFTER COUNTRY FILTER - POSSIBLE COUNTRY NAME MISMATCH!"
        );
      ...
      console.log(
        "All countries selected or no country filter - showing all results"
      );
    }
  ...
  const loadSavedUniversities = async (universities = allUniversities) => {
    try {
      ...
      const saved = universities.filter((uni) =>
        savedUniversityIds.includes(uni.id)
      );
        ...
        <div className="container mx-auto px-4">
          <div className="mb-8">
            ...
            <p className="text-muted-foreground">
              Explore universities that match your academic profile and
              preferences
            </p>
          </div>
              ...
              <CardContent className="p-6 space-y-6">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mb-4">
                    Refine your university matches
                  </p>
                </div>
                ...
                <div className="space-y-2">
                  <Label>
                    Maximum Tuition: ${filters.maxTuition.toLocaleString()}
                  </Label>
                  <Slider
              ...
              <Tabs defaultValue="all" onValueChange={setActiveTab}>
                ...
                <TabsContent value="saved" className="space-y-4 mt-4">
                  {(() => {
                    ...
                    if (filters.minMatchScore > 0) {
                      filteredSaved = filteredSaved.filter(
                        (uni) => uni.matchScore >= filters.minMatchScore
                      );
                    }
                    ...
                    if (filters.maxTuition < 50000) {
                      filteredSaved = filteredSaved.filter(
                        (uni) => uni.tuitionFees <= filters.maxTuition
                      );
                    }
                    ...
                    return filteredSaved.length > 0 ? (
                      <>
                        ...
                        {filteredSaved.length !== savedUniversities.length && (
                          <div className="text-center text-sm text-muted-foreground mt-4">
                            Showing {filteredSaved.length} of{" "}
                      ...
                      <div className="text-center py-10">
                        ...
                        <p className="text-muted-foreground">
                          {savedUniversities.length === 0
                            ? "Save universities to track them here"
                            : "Try adjusting your filters or search query"}
                        </p>
                      </div>
                    );
        ...
        <div className="container mx-auto px-4">
          <div className="flex flex-col md:flex-row justify-between items-center">
            ...
            <div className="text-sm text-muted-foreground">
              © 2025 GradPilot. Developed by Souvik Mandol, Ananya Shahrin
              Promi, and Wahid Al Azad Navid.
            </div>
          ...
          <div className="flex-1 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <div className="flex items-center">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    {university.tuitionFees != null
                      ? `$${university.tuitionFees.toLocaleString()}/year`
                      : "N/A"}
                  </p>
                </div>
              </div>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Button
                ...
                className={
                  isSaved ? "bg-yellow-500 hover:bg-yellow-600 text-white" : ""
                }
              >
