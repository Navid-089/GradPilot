...
import { FileText, CheckCircle, AlertCircle, Loader2, Upload, Download, RefreshCw } from "lucide-react"
...
export default function SopReview() {
  ...
  const handleReview = async () => {
    ...
    if (!validation.isValid) {
      setValidationErrors(validation.errors)
      showNotification("Please fix the validation errors before reviewing.", "error")
      return
    }
    ...
    try {
      ...
      showNotification(error.message || "Failed to review SOP. Please try again.", "error")
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const getProgressValue = () => {
    ...
    if (length <= maxLength) return 25 + ((length - minLength) / (maxLength - minLength)) * 50
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex gap-2">
          <Button variant="outline" onClick={handleDownload} disabled={!sopText}>
            <Download className="mr-2 h-4 w-4" />
            Download
          </Button>
          <Button variant="outline" onClick={handleClear} disabled={!sopText && !feedback}>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                ...
                <span className="text-muted-foreground">{Math.round(getProgressValue())}%</span>
              </div>
...
Remember to be authentic, specific, and compelling in your narrative."
            ...
            <Button 
              ...
              disabled={isLoading || validationErrors.length > 0 || !sopText.trim()}
              className="w-full"
            >
              {isLoading ? (
                <>
          ...
          <CardContent>
            {!hasReviewed && !isLoading && (
              <div className="text-center py-12">
                ...
                <p className="text-muted-foreground">
                  Enter your SOP text and click "Review My SOP" to get detailed feedback.
                </p>
              </div>
            )}
            ...
            {isLoading && (
              <div className="text-center py-12">
                ...
                <p className="text-muted-foreground">
                  Please wait while we review your statement for grammar, style, and clarity.
                </p>
              </div>
            )}
              ...
              <div className="space-y-4">
                ...
                {feedback.includes("No grammar or spelling errors") && (
                  <Alert>
                    ...
                    <AlertDescription>
                      Great job! Your SOP appears to be well-written with no major issues detected.
                    </AlertDescription>
                  </Alert>
                )}
              </div>
